...
export const PRODUCT_NAME = "Attache";
export const PRIMARY_COMMAND = "attache";
export const PUBLISHED_PACKAGE_NAME = "attache";
const PUBLISHED_PACKAGE_TARGET = `${PUBLISHED_PACKAGE_NAME}@latest`;
...
export async function getLatestVersion(): Promise<string | null> {
  try {
    const result = await execAsync(`npm view ${PUBLISHED_PACKAGE_NAME} version`, 10_000);
    return result || null;
  } catch {
    return null;
  }
...
/** Run `npm install -g attache@latest` and return success/failure. */
export async function performUpdate(): Promise<{ ok: boolean; output: string }> {
  try {
    const output = execSync(`npm install -g ${PUBLISHED_PACKAGE_TARGET}`, {
      encoding: "utf-8",
      timeout: 60_000,
